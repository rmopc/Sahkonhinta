                ...
                foreach (var item in jsonArray)
                {
                    ...
                    if (DateTime.TryParse(item["date"].ToString(), out date))
                    {
                        ...
                        if (date.ToShortDateString().Contains(today.AddDays(1).ToShortDateString()))
                        {
                            ...
                            pricelistTomorrow = pricelist.Where(x => x.date >= startDateTimeTomorrow && x.date <= endDateTimeTomorrow).ToList();
                            ...
                            lowPrice.Text = (dailyMinTomorrow / 10 * taxPercentage).ToString("F") + " c/kWh";
